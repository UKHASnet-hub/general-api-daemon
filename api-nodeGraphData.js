...
app.get('/nodeGraphData', function(req, res) {
    ...
    pg.connect(pgConfig, function(err, client, done) {
        ...
        var dataQuery
        if(req.query.name) {
            dataQuery = client.query('SELECT data_float.data AS data, data_float.position as pos,data_float.fieldid AS typeid, fieldtypes.name as type, fieldtypes.dataid AS unit, packet_rx.packet_rx_time AS time from ukhasnet.data_float INNER JOIN (SELECT fieldtypes.id AS id,fieldtypes.name AS name,fieldtypes.dataid AS dataid FROM ukhasnet.fieldtypes WHERE fieldtypes.type=\'Float\' AND fieldtypes.name=\'Temperature\' OR fieldtypes.name=\'Voltage\' OR fieldtypes.name=\'RSSI\' OR fieldtypes.name=\'Humidity\' OR fieldtypes.name=\'Pressure\' OR fieldtypes.name=\'Sun\') AS fieldtypes ON data_float.fieldid = fieldtypes.id INNER JOIN (SELECT id from ukhasnet.packet AS id WHERE originid = (SELECT id FROM ukhasnet.nodes WHERE name=$1)) AS packet ON packet.id = data_float.packetid INNER JOIN (SELECT packetid,packet_rx_time FROM ukhasnet.packet_rx WHERE packet_rx_time >= $2) AS packet_rx on packet.id=packet_rx.packetid ORDER BY packet_rx.packet_rx_time ASC;',[req.query.name, since_time.toISOString()])
        } else {
